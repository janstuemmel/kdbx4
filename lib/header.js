 ...
 * @property {ArrayBuffer} headerHash
 * @property {ArrayBuffer} headerHmac
 ...
 */
export const readKdfParameters = buffer => {
 ...
 */
export const readHeaderHashAndHmac = (headerSize, buffer) => [
  buffer.slice(headerSize, headerSize + 32),
  buffer.slice(headerSize + 32, headerSize + 64),
];
 ...
 * @returns {KdfParams}
 */
export const mapKdfParameters = records => {
  if (!records.$UUID) {
 ...
 * @param {[ArrayBuffer, ArrayBuffer]} headerHashAndHmac
 * @returns {Kdbx4Header}
 */
export const mapHeader = (size, headerStart, headerFields, headerHashAndHmac) => {
  const kdfParameters = readKdfParameters(headerFields.kdfParams);

  if (headerStart.signature1 !== 0x9A_A2_D9_03) {
    throw new Error('wrong signature filemagic');
  }

  if (headerStart.signature2 !== 0xB5_4B_FB_67) {
  ...
  const [headerHash, headerHmac] = headerHashAndHmac;
  ...
  return {
    ...
    kdfParams: mapKdfParameters(kdfParameters),
...
export const getHeader = buffer => {
  ...
  const headerHashAndHmac = readHeaderHashAndHmac(size, buffer);
  return mapHeader(size, headerStart, headerFields, headerHashAndHmac);
};
